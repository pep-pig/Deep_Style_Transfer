...
def parse_args(parameters):
  ...
  parser.add_argument('--original_colors', type=bool,default=False,
                      help='Transfer the style but not the colors.')
  ...
  parser.add_argument('--style_mask', action='store',
...
'''
  ...
  weight, bias = vgg['layers'(字典key)][0(去掉"虚"的维,相当于np.squeeze)][layer(层索引)][0][0(连续两次去掉虚的维度)][0(weigh, pad, type, name, stride5类信息的索引,从0开始)][0表示weight，1表示bias]
'''
...
def sum_content_losses(sess, net, content_img):
  ...
  #usage of zip(): input iterable objects ,and then return a list consists of tuple
...
def preprocess(img):
  '''
  preprocess is aimed to address two problem
  1. consider the mean value of pre-trained vgg-19
  2. bgr to rgb is consitent with opencv
  '''
  # bgr to rgb
  #this format is equal to imag[:,:,::-1] ,::-1 means converse  along the last axis
  img = img[...,::-1]
...
def postprocess(img):
  ...
  img = img[0]
  #limit the pixel value between 0-255
...
def minimize_with_lbfgs(sess, net, optimizer, init_img):
  ...
  #这个优化器是从scipy中调出来的，不同于tensorflow自带adam 优化器 所以格式上有所不同
  #这个  是optimizer.minimize(sess)
  #adam 是sess.run(optimizer.minimize(loss))
...
def start_cal(parameters,original_color):
  # try:
	global args
	...
	if original_color:
	    ...
	    args.original_colors = False
	if args.video: render_video()

	else: render_single_image()
	return 'image_output/result/result.png'
  # except:
  #   print('failed')
  #   return ''

if __name__ == '__main__':
  start_cal("--content_img content_image\lion.jpg --style_imgs style_image\oil_crop.jpg --verbose ",False)
